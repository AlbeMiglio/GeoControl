// test/e2e/measurements/getMeasurements.e2e.test.ts
...
describe("GET /networks/:networkCode/measurements", () => {
    ...
    it("200 – retrieves measurements successfully", async () => {
        ...
        expect(res.body[0]).toHaveProperty("sensorMacAddress");
        expect(res.body[0]).toHaveProperty("stats");
        expect(res.body[0]).toHaveProperty("measurements");
    });
    ...
    it("400 – bad request with invalid date format", async () => {
        const res = await request(app)
            .get(`/api/v1/networks/${networkCode}/measurements`)
            .query({
                sensorMacs: sensorMac,
    ...
    it("404 – network not found", async () => {
        const res = await request(app)
            .get(`/api/v1/networks/NON_EXISTENT/measurements`)
            .query({
                sensorMacs: sensorMac,
                startDate: "2025-02-18T15:00:00Z",
                endDate: "2025-02-18T17:00:00Z"
    ...
    it("500 - internal server error (getAllMeasurementsByNetwork)", async () => {
        const spy = jest
            .spyOn(MeasurementRepository.prototype, "getMeasurementsBySensors")
            .mockRejectedValueOnce(new Error("Internal DB error"));
